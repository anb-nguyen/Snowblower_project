...
int LaserScanner::isObstacleTooClose(sensor_msgs::LaserScan & LaserMsg, int start_index, int end_index, double DistanceThreshold){
  ...
  //int min_index = getIndexOfMiniumRange(LaserMsg);
  if (LaserMsg.ranges[min_index] < DistanceThreshold) {
    //bool measurementResult = isMeasurementReliable(LaserMsg, min_index);
    if (min_index > 300 || min_index < 60){ //measurementResult != 0
      if (min_index <= 359 && min_index >= 300) // 0 -60
        result = -1;  // right
      else if (min_index <= 60 && min_index >= 0) // 60 0
        result = 1; // left
    } 
  }
